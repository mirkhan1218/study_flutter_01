...
class MultifulImageView extends StatefulWidget {
  ...
  const MultifulImageView({super.key, this.initImages});
...
class _MultifulImageViewState extends State<MultifulImageView> {
  ...
  void initState() {
    ...
    scrollController.addListener(() {
      ...
      if (currentScroll > maxScroll - 150 && currentScroll != lastPage) {
      ...
      albums = await PhotoManager.getAssetPathList(
        ...
        filterOption: FilterOptionGroup(
          imageOption: const FilterOption(
            needTitle: true,
            sizeConstraint: SizeConstraint(minWidth: 800, minHeight: 800),
          ),
          orders: [
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const AppFont('최근 항목', fontWeight: FontWeight.bold, size: 18),
        actions: [
          GestureDetector(
            onTap: () {
